...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Create Transloadit signature
    const params = {
      auth: {
        key: TRANSLOADIT_API_KEY,
        expires: Math.floor(Date.now() / 1000) + 3600 // 1 hour from now
      },
      template_id: 'your-template-id', // You'll need to create a template
      notify_url: 'https://your-domain.com/api/collaborations/transloadit-webhook',
      fields: {
        creatorUsername,
        requestId,
        filename
