...
// Configuration
const SIZES = [192, 512];
const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'twillylaughs');
...
async function generateIcon(size) {
    ...
    <svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Fun wavy pattern -->
        <g clip-path="url(#laughMask)">
            ${Array.from({length: 8}, (_, i) => {
                const y = (i * size/8) + (size/16);
                return `
                    <path d="M0,${y} 
                           c${size/4},${-size/16} 
                            ${size/2},${size/16} 
...
async function main() {
    console.log(gradient.pastel.multiline('\n😂 Generating Twilly Laughs Icons...\n'));
