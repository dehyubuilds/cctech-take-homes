...
// Configuration
const SIZES = [192, 512];
const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'fit-logo');
    ...
    <svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(${size/2}, ${size/2})">
            ${Array.from({length: 3}, (_, i) => {
                const radius = size/5 + (i * size/15);
                return `
                    <circle cx="0" cy="0" r="${radius}"
                            stroke="${PRIMARY_COLOR}"
                            stroke-width="${size/50}"
                            stroke-dasharray="${size/12},${size/24}"
                            fill="none"
                            opacity="${0.7 - (i * 0.15)}"
        ...
        <g transform="translate(${size/2}, ${size/2})">
            ...
            <g transform="translate(0, ${size/8})">
                <rect x="${-size/3}" y="${-size/25}" 
                      width="${size/1.5}" height="${size/12.5}"
                      fill="${PRIMARY_COLOR}"
                      rx="${size/50}"
                      filter="url(#shadow)" />
                
                <circle cx="${-size/3}" cy="0" r="${size/12}"
                        fill="${ENERGY_COLOR}"
                        filter="url(#shadow)" />
                <circle cx="${size/3}" cy="0" r="${size/12}"
                        fill="${ENERGY_COLOR}"
