...
async function findDehswizzyFiles() {
  ...
  // Step 1: Find the userId for dehswizzy
  ...
  // Also try scanning all USER# entries to find one with username dehswizzy
  if (!dehswizzyUserId) {
    console.log('🔍 Scanning all USER entries for dehswizzy...\n');
    ...
    for (const item of allUsers.Items) {
      if (item.username === 'dehswizzy') {
        ...
        if (pkValue.includes('-')) {
          dehswizzyUserId = pkValue;
          console.log(`✅ Found dehswizzy userId from PK: ${dehswizzyUserId}`);
          console.log(`   PK: ${item.PK}`);
          console.log(`   SK: ${item.SK}`);
          break;
        } else if (item.userId) {
  ...
  // Step 2: Find all files with this creatorId
  ...
  if (filesWithCreatorId.length > 0) {
    ...
    filesWithCreatorId.forEach((file, idx) => {
      ...
      console.log(`   creatorId: ${file.creatorId}`);
      console.log('');
    });
