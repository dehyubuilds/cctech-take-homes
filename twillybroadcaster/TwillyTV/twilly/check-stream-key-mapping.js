...
async function checkStreamKeyMapping(streamKey) {
  try {
    ...
    if (result.Item) {
      ...
      // Check the video entry - try admin account first (for Twilly TV)
      const adminEmail = 'dehyu.sinyan@gmail.com';
      console.log(`\n🔍 Checking for videos under admin account: ${adminEmail}`);
      ...
      if (videoResult.Items && videoResult.Items.length > 0) {
        ...
        videoResult.Items.forEach((item, idx) => {
          ...
          console.log(`       folderName: ${item.folderName || 'NOT SET'}`);
        });
      } else {
        console.log(`\n⚠️ No videos found for this streamKey under admin account`);
      }
    } else {
