...
async function createChannelsNameGSI() {
  console.log('🔧 Creating GSI for Channels by Name...\n');
  ...
  try {
    ...
    const gsiExists = existingGSIs.some(gsi => gsi.IndexName === 'ChannelsByNameIndex');

    if (gsiExists) {
      console.log('✅ GSI "ChannelsByNameIndex" already exists!');
    ...
    // Check if we need to add channelName to attribute definitions
      ...
      GlobalSecondaryIndexUpdates: [
        {
          Create: {
            ...
            KeySchema: [
              ...
              {
                AttributeName: 'PK',
                KeyType: 'RANGE' // Sort key (CHANNEL#...)
              }
            ],
            Projection: {
    ...
    console.log('\n🚀 Creating GSI...');
    console.log('   Index Name: ChannelsByNameIndex');
    console.log('   Partition Key: channelName');
...
createChannelsNameGSI().catch(console.error);
