...
async function createStreamKeysGSI() {
  console.log('🔧 Creating GSI for Stream Keys by Creator...\n');
  ...
  try {
    ...
    const gsiExists = existingGSIs.some(gsi => gsi.IndexName === 'StreamKeysByCreatorIndex');

    if (gsiExists) {
      console.log('✅ GSI "StreamKeysByCreatorIndex" already exists!');
    ...
    // Check if we need to add creatorId to attribute definitions
    const existingAttributes = tableDescription.Table.AttributeDefinitions.map(a => a.AttributeName);
    const needsCreatorId = !existingAttributes.includes('creatorId');
    ...
    if (needsCreatorId) {
      ...
      console.log('➕ Adding creatorId to attribute definitions');
    }
      ...
      GlobalSecondaryIndexUpdates: [
        {
          Create: {
            ...
            KeySchema: [
              ...
              {
                AttributeName: 'SK',
                KeyType: 'RANGE' // Sort key (MAPPING or channel-specific)
              }
            ],
            Projection: {
    ...
    console.log('\n🚀 Creating GSI...');
    console.log('   Index Name: StreamKeysByCreatorIndex');
    console.log('   Partition Key: creatorId');
...
createStreamKeysGSI().catch(console.error);
