...
if aws lambda get-function --function-name "$FUNC_NAME" --region "$REGION" 2>/dev/null; then
  ...
  aws lambda create-function --function-name "$FUNC_NAME" \
    ...
    --environment "Variables={CBB_API_BASE_URL=$CBB_API_BASE_URL,CBB_API_KEY=$CBB_API_KEY,STATICS_BASE_URL=$STATICS_BASE_URL,STATICS_API_KEY=$STATICS_API_KEY,STATICS_TABLE_NAME=$STATICS_TABLE_NAME,TWILIO_ACCOUNT_SID=$TWILIO_ACCOUNT_SID,TWILIO_AUTH_TOKEN=$TWILIO_AUTH_TOKEN,TWILIO_PHONE_NUMBER=$TWILIO_PHONE_NUMBER,DAILY_PICKS_TABLE=march_madness_daily_picks,PREDICTIONS_LOG_TABLE=march_madness_predictions_log}" \
    --region "$REGION"
fi
